...
def iou(y_true, y_pred):
    intersection = (y_true * y_pred).sum()  # 计算交集部分
    union = y_true.sum() + y_pred.sum() - intersection  # 计算并集部分
    return (intersection + 1e-15) / (union + 1e-15)  # 防止除以0

def dice(y_true, y_pred):
    intersection = (y_true * y_pred).sum()  # 计算交集部分
    return (2 * intersection + 1e-15) / (y_true.sum() + y_pred.sum() + 1e-15)  # 计算Dice系数
