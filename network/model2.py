...
from network.VSS import VSSAdapter
...
class SemanticConditionModulator(nn.Module):
    """
    Semantic-conditioned modulation module
    Generate per-pixel scale (gamma) and bias (beta)
    from foreground and background semantic cues.
    """
    ...
    def forward(self, fg, bg):
        ...
        return:
            gamma, beta: [B, H, W, C]
        """
        cond = torch.cat([fg, bg], dim=-1)
        gamma_beta = self.mlp(cond)
        gamma, beta = gamma_beta.chunk(2, dim=-1)
        return gamma, beta


class SemanticConditionedFeatureAggregation(nn.Module):
    """
    SCFA: Semantic-Conditioned Feature Aggregation
    Drop-in replacement for CDFA
    """
    def __init__(
        self,
        in_c,
        ...
        # semantic condition modulator (core innovation)
        self.cond_modulator = SemanticConditionModulator(dim)
    ...
    def forward(self, x, fg, bg):
        """
        x  : [B, C, H, W]  main feature
        fg : [B, C, H, W]  foreground semantic cue
        bg : [B, C, H, W]  background semantic cue
        """

        # ---- input adaptation ----
        x = self.input_cbr(x)

        # to channel-last for semantic conditioning
        ...
        # ---- value projection ----
        v = self.value_proj(x)                  # [B, H, W, C]
        v = v.permute(0, 3, 1, 2).contiguous()  # [B, C, H, W]

        # ---- local aggregation (no attention) ----
        # unfold: [B, C * K*K, L]
        v_unfold = self.unfold(v)

        # reshape to [B, C, K*K, L]
        v_unfold = v_unfold.view(
            B,
            C,
            self.kernel_size * self.kernel_size,
            -1
        ...
        # simple but stable local aggregation
        v_local = v_unfold.mean(dim=2)  # [B, C, L]
        ...
        x_agg = x_agg.permute(0, 2, 3, 1).contiguous()  # [B,H,W,C]

        # ---- semantic-conditioned modulation (核心) ----
        gamma, beta = self.cond_modulator(fg, bg)
        x_cond = gamma * x_agg + beta

        # ---- residual & output ----
        x_out = x + x_cond
        x_out = x_out.permute(0, 3, 1, 2).contiguous()
        x_out = self.output_cbr(x_out)

        return x_out



class ConDSeg2(nn.Module):
    def __init__(self, H=256, W=256):
        ...
        # self.vss0 = VSSAdapter(build_stage_vssm(in_chans=64, embed_dim=64, depth=3))
        # self.vss1 = VSSAdapter(build_stage_vssm(in_chans=256, embed_dim=256, depth=3))
        # self.vss2 = VSSAdapter(build_stage_vssm(in_chans=512, embed_dim=512, depth=3))
        # self.vss3 = VSSAdapter(build_stage_vssm(in_chans=1024, embed_dim=1024, depth=3))

        #(1,1,2,2)
        self.vss0 = VSSAdapter(dim=64, out_dim=64, ffn_ratio=3.0)
        self.vss1 = VSSAdapter(dim=256, out_dim=256, ffn_ratio=3.0)
        self.vss2 = VSSAdapter(dim=512, out_dim=512, ffn_ratio=3.0)
        self.vss3 = VSSAdapter(dim=1024, out_dim=1024, ffn_ratio=3.0)
