...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 9, 2], depths_decoder=[2, 9, 2, 2],
                 dims=[64, 256, 512, 1024],  d_state=16, drop_rate=0.,
...
def build_stage_vssm(
    ...
    return VSSM(
        patch_size=1,          # ⭐ 不改变空间分辨率
        in_chans=in_chans,     # ⭐ 接 ResNet feature
        num_classes=0,         # 不用分类头
        depths=[depth],        # 只有 1 个 stage
        depths_decoder=[0],
        dims=[embed_dim],
        dims_decoder=[embed_dim],
        d_state=d_state,
        drop_rate=0.,
        attn_drop_rate=0.,
        drop_path_rate=0.,
...
class VSSAdapter(nn.Module):
    """
    包装 build_stage_vssm 输出的 VSSM 模块
    - NHWC ↔ NCHW
    - 可选 residual
    """

    def __init__(self, vssm, use_residual=True):
        super().__init__()
        self.vssm = vssm
        self.use_residual = use_residual
        self.proj = None  # 如果通道不匹配，动态创建 1x1 conv

    def forward(self, x):
        x_in = x  # NCHW

        # ⭐ 修复：直接使用 forward_backbone 而不是 forward_features
        # forward_backbone 内部处理了 NCHW 到 NHWC 的转换
        x = self.vssm.forward_backbone(x)  # 直接传入 NCHW 格式

        # forward_backbone 输出已经是 NHWC，需要转回 NCHW
        x = x.permute(0, 3, 1, 2).contiguous()  # NHWC -> NCHW
        # print(f"[VSSAdapter] Input shape: {x_in.shape}, Output shape: {x.shape}")
        # print(f"[VSSAdapter] Output min/max/mean: {x.min().item():.4f}/{x.max().item():.4f}/{x.mean().item():.4f}")

        if self.use_residual:
            if x.shape[1] != x_in.shape[1]:
                # if self.proj is None:
                #     self.proj = nn.Conv2d(x.shape[1], x_in.shape[1], 1).to(x.device)
                x = self.proj(x)
            x = x + x_in
        # if self.use_residual:
        #     print(f"[VSSAdapter] Residual fused output shape: {x.shape}")
        #     print(
        #         f"[VSSAdapter] Residual fused min/max/mean: {x.min().item():.4f}/{x.max().item():.4f}/{x.mean().item():.4f}")
