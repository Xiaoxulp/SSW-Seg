...
class dilated_conv(nn.Module):
    def __init__(self, in_c, out_c):
        ...
        # 多尺度 dilated conv（完全保留你的设计）
        self.c1 = CBR(in_c, out_c, kernel_size=1, padding=0)
        self.c2 = CBR(in_c, out_c, kernel_size=3, padding=6, dilation=6)
        self.c3 = CBR(in_c, out_c, kernel_size=3, padding=12, dilation=12)
        self.c4 = CBR(in_c, out_c, kernel_size=3, padding=18, dilation=18)

        # 融合
        self.c5 = CBR(out_c * 4, out_c, kernel_size=3, padding=1, act=False)
        self.c6 = CBR(in_c, out_c, kernel_size=1, padding=0, act=False)

        # ★ 用 GBC 代替 channel + spatial attention
        self.gbc = GBC(out_c)
