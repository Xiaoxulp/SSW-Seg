...
class VSSAdapter(nn.Module):
    """VSS Adapter with pre and post adapters"""
    def __init__(self, dim, out_dim=None, **kwargs):
        super().__init__()
        mamba_dim = out_dim if out_dim is not None else dim

        # pre_adapter: [B,C,H,W] -> [B,H,W,C]
        self.pre_adapter = CBR1(dim, mamba_dim, kernel_size=1, padding=0, act=True)
        # VSSBlock expects [B,H,W,C] as input
        self.vss = VSSBlock(hidden_dim=mamba_dim, **kwargs)
        # post_adapter: [B,H,W,C] -> [B,C,H,W]
        self.post_adapter = CBR2(mamba_dim, dim, kernel_size=1, padding=0, act=True)

    def forward(self, x):
        x_m = self.pre_adapter(x)    # [B,C,H,W] -> [B,H,W,C]
        x_m = self.vss(x_m)          # VSSBlock
        x = self.post_adapter(x_m)   # [B,H,W,C] -> [B,C,H,W]
        return x
