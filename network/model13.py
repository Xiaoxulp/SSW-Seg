...
#
#         # -------- 融合卷积 --------
#         self.c1 = CBR(in_c + out_c, out_c, kernel_size=1, padding=0)  # 有 skip
#         self.c1_no_skip = CBR(in_c, out_c, kernel_size=1, padding=0)  # 无 skip
#
#         # -------- 残差卷积块 --------
#         self.c2 = CBR(out_c, out_c, act=False)
#         self.c3 = CBR(out_c, out_c, act=False)
...
class decoder_block(nn.Module):
    def __init__(self, in_c, out_c, scale=2):
        ...
        # -------- 上采样 --------
        # 用于将输入特征图上采样
        self.up_x = nn.Upsample(scale_factor=scale,
                                mode="bilinear",
                                align_corners=True)

        # 用于将skip特征图上采样（如果需要）
        self.up_skip = nn.Upsample(scale_factor=scale,
                                   mode="bilinear",
                                   align_corners=True)
    ...
    def forward(self, x, skip=None):
        # ===== 上采样输入 =====
        x = self.up_x(x)  # 例如：16×16 → 32×32

        # ===== 处理skip =====
        if skip is not None:
            # 将skip也上采样到和x相同的尺寸
            if skip.shape[2:] != x.shape[2:]:
                skip = self.up_skip(skip)  # 例如：16×16 → 32×32
...
class ConDSeg13(nn.Module):
    def __init__(self, H=256, W=256):
        ...
        """ Decoder """
        # self.dec4 = decoder_block(128, 128)
        # self.dec3 = decoder_block(128, 128)
        # self.dec2 = decoder_block(128, 128)
        # self.dec1 = decoder_block(128, 128)
        self.dec4 = decoder_block(128, 128, scale=2)  # 8×8 → 16×16
        self.dec3 = decoder_block(128, 128, scale=2)  # 16×16 → 32×32
        self.dec2 = decoder_block(128, 128,  scale=2)  # 32×32 → 64×64
        self.dec1 = decoder_block(128, 128, scale=2)  # 64×64 → 128×128
        """ Output Block """
