...
from network.MALA import AddLinearAttention, RoPE
from network.VSS import SS2D
...
class ContrastDrivenFeatureAggregation(nn.Module):
    ...
    def __init__(self, in_c, dim, num_heads, kernel_size=3, padding=1, stride=1,
        ...
        # 修改：使用AddLinearAttention增强特征，但保留原注意力计算
        self.fg_enhance = AddLinearAttention(dim, num_heads)  # 用于增强前景特征
        self.bg_enhance = AddLinearAttention(dim, num_heads)  # 用于增强背景特征
    ...
    def forward(self, x, fg, bg):
        ...
        # 修改：先用AddLinearAttention增强特征，再计算注意力
    ...
    def compute_attention(self, feature_map, B, H, W, C, feature_type):
        """改进：先用AddLinearAttention增强特征，再计算注意力权重"""
        attn_linear = self.attn_fg if feature_type == 'fg' else self.attn_bg
        enhance_layer = self.fg_enhance if feature_type == 'fg' else self.bg_enhance
        ...
        feature_map_pooled = self.pool(feature_map.permute(0, 3, 1, 2))

        # 🔴 关键修改：先用AddLinearAttention增强特征
        enhanced_feature = enhance_layer(feature_map_pooled, sin, cos)  # [B, C, h, w]

        # 将增强后的特征转换回原格式
        enhanced_feature = enhanced_feature.permute(0, 2, 3, 1)  # [B, h, w, C]

        # 使用原线性层计算注意力权重
        attn = attn_linear(enhanced_feature).reshape(
