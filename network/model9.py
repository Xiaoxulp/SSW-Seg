...
class DecoupleLayer(nn.Module):
    def __init__(self, in_c=1024, out_c=256):
        ...
        # 共享编码（更稳定）
        self.share = CBR(in_c, out_c, kernel_size=3, padding=1)

        # 各自细化
        self.cbr_fg = CBR(out_c, out_c, kernel_size=3, padding=1)
        self.cbr_bg = CBR(out_c, out_c, kernel_size=3, padding=1)
        self.cbr_uc = CBR(out_c, out_c, kernel_size=3, padding=1)

        # 门控（使用 ReLU 激活，避免梯度消失问题）
        self.fg_gate = nn.Conv2d(out_c, 1, kernel_size=1)
        self.bg_gate = nn.Conv2d(out_c, 1, kernel_size=1)
        self.uc_gate = nn.Conv2d(out_c, 1, kernel_size=1)
    ...
    def forward(self, x):
        ...
        g_fg = self.relu(self.fg_gate(s))
        g_bg = self.relu(self.bg_gate(s))
        g_uc = self.relu(self.uc_gate(s))

        # 🔥 关键设计：添加小常数避免数值不稳定性
        epsilon = 1e-6  # 防止零除错误
        f_fg = fg * (1 - g_bg + epsilon) + g_uc * uc
        f_bg = bg * (1 - g_fg + epsilon) + g_uc * uc
...
class AuxiliaryHead(nn.Module):
    def __init__(self, in_c):
        ...
        def make_branch():
            return nn.Sequential(
                CBR(in_c, 256, 3, padding=1),
                nn.ConvTranspose2d(256, 256, kernel_size=4, stride=2, padding=1),  # 使用 ConvTranspose2d 替代 Upsample
                CBR(256, 256, 3, padding=1),
                nn.ConvTranspose2d(256, 256, kernel_size=4, stride=2, padding=1),
                CBR(256, 128, 3, padding=1),
                nn.ConvTranspose2d(128, 128, kernel_size=4, stride=2, padding=1),
                CBR(128, 64, 3, padding=1),
                nn.ConvTranspose2d(64, 64, kernel_size=4, stride=2, padding=1),
                CBR(64, 64, 3, padding=1),
                nn.Conv2d(64, 1, 1),
                nn.Sigmoid()
            )

        self.branch_fg = make_branch()
        self.branch_bg = make_branch()
        self.branch_uc = make_branch()
    ...
    def forward(self, f_fg, f_bg, f_uc):
        ...
        # 🔥 让“不确定区域”自动降低 fg/bg 置信度
        mask_fg = mask_fg * (1 - mask_uc)
        mask_bg = mask_bg * (1 - mask_uc)
...
# 对最下面一层进行创新 修改了前景 背景 不确定区域
class ConDSeg9(nn.Module):
