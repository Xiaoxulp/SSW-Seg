...
# #RoPE 是一种用于生成旋转位置编码的机制，通常用于像 Transformer 这样的模型中以增强模型对序列位置的理解。
# # 该实现的目标是通过 sin 和 cos 函数计算并返回旋转位置编码的正弦和余弦值
...
class VSSM(nn.Module):
    ...
    def forward(self, x, to_dtype=False, force_fp32=False):
        ...
        y = self._cross_merge(ys.reshape(B, K, -1, H, W).contiguous(), paths_lr, paths_tb, reverse_lr,
                              reverse_tb).reshape(B, -1, L).contiguous()
