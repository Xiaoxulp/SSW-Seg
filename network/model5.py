...
class GBC_Decouple_CBAM(nn.Module):
    def __init__(self, in_channels, out_channels, norm_type='GN'):
        ...
        # 2. 通道 + 空间注意力
        self.channel_att = channel_attention(out_channels)
        self.spatial_att = spatial_attention(kernel_size=7)

        # 3. 通道降维：保证 GBC 输出通道 = Decouple 输出通道
        self.gbc_reduce = nn.Conv2d(in_channels, out_channels, kernel_size=1, bias=False)

        # 4. 解耦分支
        self.decouple = DecoupleLayer(in_c=out_channels, out_c=out_channels)

    def forward(self, x):
        # 1. GBC增强特征
        gbc_feat = self.gbc(x)  # [B, in_channels, H, W]
        gbc_feat = self.gbc_reduce(gbc_feat)  # [B, out_channels, H, W]

        # 2. 通道 + 空间注意力
        ca = self.channel_att(gbc_feat)  # [B,out_channels,1,1] 或 [B,out_channels,H,W]
        sa = self.spatial_att(gbc_feat)  # [B,1,H,W]
        att_feat = ca * sa  # [B,out_channels,H,W]

        # 3. 解耦分支
        f_fg, f_bg, f_uc = self.decouple(gbc_feat)

        # 4. 门控交互
        f_fg = f_fg * att_feat
        f_bg = f_bg * (1 - att_feat)
        f_uc = f_uc * (att_feat * (1 - att_feat))

        return f_fg, f_bg, f_uc

class ConDSeg5(nn.Module):
    def __init__(self, H=256, W=256):
        ...
        self.decouple_layer = GBC_Decouple_CBAM(1024, 128)
