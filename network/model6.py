...
class FreqAwareDecouple(nn.Module):
    """
    创新组合：
    1. 先 DecoupleLayer 解耦语义
    2. 每个分支独立 WTConv2d 进行频率增强
    """
    def __init__(self, in_c=1024, out_c=256, wt_levels=1, wt_type='db1'):
        super(FreqAwareDecouple, self).__init__()
        self.decouple = DecoupleLayer(in_c, out_c)

        # 每个分支独立的 WTConv2d
        self.wt_fg = WTConv2d(out_c, out_c, kernel_size=3, stride=1, wt_levels=wt_levels, wt_type=wt_type)
        self.wt_bg = WTConv2d(out_c, out_c, kernel_size=3, stride=1, wt_levels=wt_levels, wt_type=wt_type)
        self.wt_uc = WTConv2d(out_c, out_c, kernel_size=3, stride=1, wt_levels=wt_levels, wt_type=wt_type)

    def forward(self, x):
        # Step 1: 语义解耦
        f_fg, f_bg, f_uc = self.decouple(x)

        # Step 2: 分支频率增强
        f_fg = self.wt_fg(f_fg)
        f_bg = self.wt_bg(f_bg)
        f_uc = self.wt_uc(f_uc)
...
class ConDSeg6(nn.Module):
    def __init__(self, H=256, W=256):
        ...
        """ Decouple Layer """
        self.decouple_layer = FreqAwareDecouple(1024, 128)
