...
class ContrastDrivenFeatureAggregation(nn.Module):
    ...
    def compute_attention(self, feature_map, B, H, W, C, feature_type):
        ...
        # 下采样
        feature_map_pooled = self.pool(feature_map.permute(0, 3, 1, 2))  # [B,C,h,w]

        # ------- ★ CGLA：全局对比 AddLinearAttention -------
        sin, cos = self.rope((h, w))

        global_feat = enhance_layer(feature_map_pooled, sin, cos)  # [B,C,h,w]

        # 门控（限制到 0~1）
        gate = torch.sigmoid(global_feat)

        # 用 gate 调制 pooled 特征（强调关键区域）
        fused = feature_map_pooled * (1 + gate)

        # 恢复维度
        fused = fused.permute(0, 2, 3, 1)

        # ------- 原注意力权重生成（局部） -------
        attn = attn_layer(fused).reshape(
