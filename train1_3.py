...
from network_pvt.model import ConDSeg_pvt
...
#这是创新之后的预训练
    pretrained_backbone = "/home/liuyameng/ConDSeg-main/run_files/Glas/stage2_Glas_None_lr0.0001_20251228-152704/checkpoint.pth"
    ...
    #     A.RandomResizedCrop(
    #         size=size,
    #         scale=(0.8, 1.0),  # 裁剪面积占原图比例范围
    #         ratio=(0.9, 1.1),  # 裁剪宽高比范围
    #         p=1.0  # 每次应用该变换的概率
    #     ),
    #     # A.CoarseDropout(p=0.3, max_holes=10, max_height=32, max_width=32)
    #     A.CoarseDropout(
    ...
    if pretrained_backbone:
        ...
        for name, param in model.named_parameters():
            if name.startswith('layer0') or name.startswith('layer1') or name.startswith('vss0')or name.startswith('vss1') or name.startswith('vss3') or name.startswith('vss2') or name.startswith('layer2') or name.startswith(
                    'layer3'):
    ...
    for name, param in model.named_parameters():
        if name.startswith('layer0') or name.startswith('vss3') or name.startswith('vss2') or name.startswith('vss1') or name.startswith('vss0') or name.startswith('layer1') or name.startswith('layer2') or name.startswith(
                'layer3'):
