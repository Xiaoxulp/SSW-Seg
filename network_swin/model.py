...
from network_swin.swin_Transfomer import SwinTransformer
...
class ConDSeg_swin(nn.Module):
    def __init__(self, H=256, W=256):
        ...
        self.backbone = SwinTransformer(
            img_size=128,  # 你的输入尺寸
            patch_size=4,
            in_chans=3,  # 医学图像可以是 1

            embed_dim=96,  # Swin-T/小型版本也

        )
        self.align1 = nn.Conv2d(96, 64, 1)
        self.align2 = nn.Conv2d(192, 128, 1)
        self.align3 = nn.Conv2d(384, 320, 1)
        self.align4 = nn.Conv2d(768, 512, 1)
    ...
    def forward(self, image):
        feats = self.backbone(image)  # Swin 输出 4 个 stage

        f1, f2, f3, f4 = feats

        # Swin: [B, H, W, C] → Conv2d 需要 [B, C, H, W]
        f1 = f1.permute(0, 3, 1, 2).contiguous()  # [B, 96,  H/4,  W/4]
        f2 = f2.permute(0, 3, 1, 2).contiguous()  # [B, 192, H/8,  W/8]
        f3 = f3.permute(0, 3, 1, 2).contiguous()  # [B, 384, H/16, W/16]
        f4 = f4.permute(0, 3, 1, 2).contiguous()  # [B, 768, H/32, W/32]

        # 通道对齐
        x1 = self.align1(f1)  # [B, 64,  H/4,  W/4]
        x2 = self.align2(f2)  # [B, 128, H/8,  W/8]
        x3 = self.align3(f3)  # [B, 320, H/16, W/16]
        x4 = self.align4(f4)  # [B, 512, H/32, W/32]



        print(f"x1: {x1.shape}")  # 应该是 [4, 64, 32, 32]
        print(f"x2: {x2.shape}")  # 应该是 [4, 128, 16, 16]
        print(f"x3: {x3.shape}")  # 应该是 [4, 320, 8, 8]
        print(f"x4: {x4.shape}")  # 应该是 [4, 512, 4, 4]
